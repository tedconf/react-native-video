  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) 0), dispatch_get_main_queue(), ^{
    ...
    [self playerItemForSource:source withCallback:^(AVPlayerItem * playerItem) {
        if (self->_rctVideoDelegate == nil ||
            ![self->_rctVideoDelegate respondsToSelector:@selector(willSetupPlayerItem:forSource:)] ||
            ![self->_rctVideoDelegate willSetupPlayerItem:playerItem forSource:source])
        {
...
-(void) setupPlayerItem:(AVPlayerItem *) playerItem forSource:(NSDictionary *) source withPlayer:(AVPlayer *) player {
    ...
    // Depending on how the delegate handles the PlayerItem,
    // Delegates may have to call this method to make sure RCTVideo works properly
    // [self observeValueForKeyPath:statusKeyPath ofObject:playerItem change:nil context:nil];
